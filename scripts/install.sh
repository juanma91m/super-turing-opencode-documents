...
Options:
  ...
  --runtime-dir <path>   Managed Quarto runtime root
...
write_marker() {
  ...
  local version runtime_version runtime_path
  ...
  python3 - "$marker" "$REPO_DIR" "$version" "$runtime_version" "$runtime_path" "$ASSETS_ONLY" <<'PY'
import datetime
import json
import pathlib
import sys

marker, repo, version, runtime_version, runtime_path, assets_only = sys.argv[1:]
payload = {
    "addonId": "documents",
    "version": version,
