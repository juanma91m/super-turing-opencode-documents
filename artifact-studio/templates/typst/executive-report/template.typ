#let artifact-document(meta: (:), theme: (:), mode: "report", logo: none, body) = {
  ...
  set text(font: ("IBM Plex Sans", "Noto Sans"), size: if mode == "form" { 9pt } else { 10.75pt }, fill: theme.text, lang: "es")
  set par(leading: if mode == "form" { 0.6em } else { 0.85em }, justify: false)
  set heading(numbering: "1.1", outlined: true)
  show heading.where(level: 1): it => if mode == "form" { block(above: 24pt, below: 9pt, breakable: false)[#text(size: 13pt, weight: "semibold", fill: theme.text)[#it.body] #v(-9pt) #line(length: 100%, stroke: .7pt + theme.accent)] } else { block(above: 18pt, below: 13pt, breakable: false)[#text(size: 19pt, weight: "semibold", fill: theme.text)[#it.body] #line(length: 100%, stroke: .7pt + theme.accent)] }
...
#let callout(tone, title, content, theme) = block(above: 7pt, below: 10pt, fill: theme.at(tone), stroke: (left: 2pt + theme.accent), inset: 10pt, radius: 2pt, breakable: false)[
