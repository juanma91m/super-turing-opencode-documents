...
function blockToTypst(block: Block, asset: string | undefined, themeVar = "theme"): string {
  switch (block.type) {
    case "paragraph": return `#block(below: 11pt)[#text(${s(block.text)})]`;
    ...
    case "bullets": return `#block(above: 5pt, below: 11pt)[${block.items.map((x) => `- #text(${s(x)})`).join("\n")}]`;
    case "numberedList": return `#block(above: 5pt, below: 11pt)[${block.items.map((x) => `+ #text(${s(x)})`).join("\n")}]`;
    case "quote": return block.attribution ? `#quote(block: true, attribution: ${s(block.attribution)})[#text(${s(block.text)})]` : `#quote(block: true)[#text(${s(block.text)})]`;
...
export class TypstRenderer implements ArtifactRenderer {
  ...
  async render(ctx: RenderContext): Promise<string> {
    ...
    const sourceList = ctx.spec.sources.length ? `\n\n#pagebreak(weak: true)\n\n= Fuentes\n${ctx.spec.sources.map((source) => `- #strong[#text(${s(`${source.sample ? "[SAMPLE] " : ""}${source.title}`)})]${source.publisher ? ` #text(${s(` — ${source.publisher}`)})` : ""}${source.url ? ` #linebreak() #link(${s(source.url)})[#text(${s(source.url)})]` : ""}`).join("\n")}` : "";
